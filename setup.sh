...
if [ $1 == "local" ]; then
  docker run -p 5500:5500 --gpus all -it -v `pwd`:/mnt --device=/dev/video0 -v $XSOCK:$XSOCK -v $XAUTH:$XAUTH -e XAUTHORITY=$XAUTH blurred
else
  {
  docker run -p 5500:5500 --gpus all -it -v `pwd`:/mnt -v $XSOCK:$XSOCK -v $XAUTH:$XAUTH -e XAUTHORITY=$XAUTH blurred
  } || {
